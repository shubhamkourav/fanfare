...
interface CareerProps extends JobProps {
    relatedJobs: JobProps[];
}

export default function Post({ details, description, relatedJobs }: CareerProps) {
    const path = usePathname();
    
    const jobLink = path.split('/').slice(0,-1).join('/')
            ...
            <div className="container">
                <div className="row lg:gx-4">
                        ...
                        <div className="career-single-content rounded-xl bg-white p-7 shadow-lg lg:px-12 lg:py-[60px]">
                            {details?.map((detail, index) => (
                                <React.Fragment key={index}>
                                    <h5 className="h5">{detail.label}</h5>
                                    <p>{detail.text}</p>
                                </React.Fragment>
                            ))}
                        </div>
                    ...
                    <div className="career-single-sidebar mt-8 lg:col-4 lg:mt-0">
                        <div className="mb-8 rounded-xl bg-white py-10 px-7 shadow-lg">
                            <h5 className="h5">Apply today</h5>
                            <p className="mt-4">{description}</p>
                            <a className="btn btn-primary mt-6 block w-full" href="#">Apply Now</a>
                        </div>
                        {
                            relatedJobs.map((job) => {
                                return (
                                    <div key={job.id} className="mb-8 rounded-xl bg-white py-10 px-7 shadow-lg">
                                        ...
                                        <ul className="mt-6 flex flex-wrap items-center text-dark">
                                            ...
                                            <li className="my-1 mr-8">
                                                <Link
                                                    ...
                                                    href={`${jobLink}/${job.id}`}
                                                >Read More
                                                    <img className="ml-1.5" src="/images/icons/arrow-right.svg" alt="" />
                                                </Link>
                                            </li>
                                        </ul>
                                    </div>
