...
export default function Jobs() {
  ...
  const job_list = useMemo(() => {
    return category ? jobs.filter(job => job.category === category) : jobs

  }, [jobs, category]);

  const onActive = (cate: String | null) => {
    setCategory(cate)
  }
      ...
      <div className="container">
        <div className="row">
          <div className="mx-auto text-center lg:col-8">
            <h2>{potions.title}</h2>
            <p className="mt-4">{potions.description}</p>
            <ul className="filter-list mt-8 flex flex-wrap items-center justify-center">
              <li onClick={() => onActive(null)}>
                <span className={`filter-btn btn btn-sm ${category === null ? 'filter-btn-active' : ''}`}>All Categories</span>
              </li>
              {
                categories.map((cate, index) => (
                  <li key={index} onClick={() => onActive(cate)}>
                    <span className={`filter-btn btn btn-sm ${cate === category ? 'filter-btn-active' : ''}`} >{cate}</span>
                  </li>
                ))
              }
            </ul>
        ...
        <div className="row mt-12">
          {
            job_list.map((job) => (
              <div key={job.id} className="mb-8 md:col-6">
